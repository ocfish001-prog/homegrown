...
export default function EventDetailModal({ event, onClose }: EventDetailModalProps) {
  ...
  function handleSaveToCalendar() {
    if (!event) return
    // Build a minimal HomegrownEvent-compatible object for the calendar API
    const calEvent = {
      id: event.id,
      title: event.title,
      date: event.date,
    ...
    const url = `/api/calendar?event=${encodeURIComponent(JSON.stringify(calEvent))}`
    window.open(url, '_blank')
  }

  function handleGoogleCalendar() {
    if (!event) return
    // getGoogleCalendarUrl expects HomegrownEvent shape; EventCardData is compatible for the fields it uses
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    window.open(getGoogleCalendarUrl(event as any), '_blank')
  }

  if (!mounted || !event) return null

  const gradient = CATEGORY_GRADIENTS[event.category] ?? 'from-sage/15 to-mauve/10'
